...
function coincident(e0, e1) {
  ...
  if (Math.abs(m0) === Infinity && Math.abs(m1) === Infinity) {
    // If these lines don't have the same x intercept, they're not coincident.
    if (!eqEpsilon(e0.left().x, e1.left().x)) {
      return false;
    }
